...
def ellipse_algebraic():
    ...
    # xp value are generated by arange function, and then used to generate yp
    ...
    # The last leg has to be drawn outside loop
...
def ellipse_disc_trig():
    ...
    for p in np.arange(p1, p2, dp):
        # These create only one quarter of the ellipse
        xp = np.abs(a * b * (b * b + a * a * (np.tan(p)) ** 2.) ** -.5)
        yp = np.abs(a * b * (a * a + b * b / (np.tan(p) ** 2.)) ** -.5)

        # Creates values that draw on both sides of the y-axis
        if p > np.pi / 2:
...
def ellipse_arc():
    ...
    p1 = 0  # Do not change from 0. That is the starting angle
    p2 = 180. * np.pi / 180.  # Ending angle in radians. [0 to +/-180]
    dp = (p2 - p1) / 300.  # Number of line parts on each top and bottom half [10-1000] <300 shows hole
